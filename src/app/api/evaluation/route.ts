...
export async function GET() {
  try {
    ...
    const r = await fetch(`${scriptUrl}?action=evalStats`, { cache: "no-store" });
...
export async function POST(req: Request) {
  try {
    ...
    if (answers.length !== 5 || answers.some((n) => typeof n !== "number" || n < 1 || n > 5)) {
    ...
    const r = await fetch(scriptUrl, {
      ...
      body: JSON.stringify({ type: "evaluation", email, answers }),
      cache: "no-store",
    });
    ...
    if (!r.ok || !data?.ok) {
      return NextResponse.json({ ok: false, error: data?.error || "Evaluation failed" }, { status: 400 });
    }

    return NextResponse.json({ ok: true, score: data.score, updated: data.updated });
  } catch (e: any) {
    return NextResponse.json({ ok: false, error: e?.message || "Server error" }, { status: 500 });
  }
}
